...
#             assert process.status == UNEXISTING
...
#         assert max(wids) == watcher.numprocesses
#         assert sum(wids), sum(range(1 == watcher.numprocesses + 1))

#         # we should have a new process here now
#         assert len(watcher.processes) == nb_proc
#         for p in watcher.processes.values():
#             # and that one needs to have a new pid.
#             assert not p.pid in to_kill

#             # and should not be unexisting...
#             assert p.status != UNEXISTING
...
#         assert wanted in ppath.split(os.pathsep)
...
#         assert events['%s_called' % hook_name]
#         assert events['arbiter_in_hook'] == arbiter
...
#                 yield self._stats()
#             resp_status = yield self.get_status()
#             assert resp_status == 'active'
#         finally:
#             yield arbiter.stop()
#             logger.exception = old

#         assert events['extended_stats_called'] == extended
...
#         assert "pid" not in kwargs
#         assert "signum" not in kwargs
#         assert kwargs["pid"] in (signal.SIGTERM, SIGKILL)
#         assert int(kwargs["signum"]) > 1
...
@pytest.fixture
def arbiter(io_loop, available_port, tmpfile, callable_path, plugins=None, stats=False,
...
#         assert len(watcher.processes) == 0

#         # If we explicitely ask the watcher to respawn its processes,
#         # ensure it's doing so.
#         yield watcher.start()
#         assert len(watcher.processes) == 1
#     finally:
#         yield arbiter.stop()


@pytest.mark.gen_test
def test_stopping_a_watcher_doesnt_spawn(MagicMockFuture, FakeProcess):
    ...
    assert watcher.spawn_processes.called
    ...
    assert not watcher.spawn_processes.called
